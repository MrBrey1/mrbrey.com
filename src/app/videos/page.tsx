      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-10 max-w-4xl">
          ...
          <a
            ...
            className="mt-7 inline-block rounded-full border border-white/15 px-5 py-2.5 text-sm font-bold text-neutral-200 hover:border-lime-400 hover:text-lime-400"
          >
        ...
        <section className="mt-16">
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-3">
            {campaignVideos.map((video) => (
              <article key={video.youtubeId} className="group overflow-hidden rounded-3xl border border-lime-400/20 bg-neutral-950 shadow-2xl shadow-lime-400/5">
                <iframe
        ...
        <section className="mt-20 border-t border-white/10 pt-14">
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-neutral-500">Canal oficial</p>
            ...
            <p className="max-w-xl text-sm leading-relaxed text-neutral-500">
              {isAutomatic
                ? "Esta sección se actualiza automáticamente con las publicaciones más recientes del canal oficial de Mr Brey."
            ...
            {galleryVideos.map((video) => {
              ...
              return (
                <article key={video.youtubeId} className="group overflow-hidden rounded-3xl border border-white/10 bg-neutral-950">
                  ...
                  <div className="p-5">
                    {published ? (
                      <p className="mb-2 text-xs font-bold uppercase tracking-[0.18em] text-neutral-600">{published}</p>
                    ) : null}
                    <h3 className="line-clamp-2 text-lg font-black group-hover:text-lime-400">{video.title}</h3>
                    <a
                      href={`https://www.youtube.com/watch?v=${video.youtubeId}`}
                      target="_blank"
                      rel="noopener noreferrer"
